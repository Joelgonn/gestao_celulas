...
export async function adicionarMembro(membroData: Omit<Membro, 'id' | 'created_at' | 'celula_id'>): Promise<void> {
    ...
    if (!isAuthorized || !celulaId) { throw new Error("Não autorizado ou célula não definida."); }
    const { error } = await supabase.from('membros').insert({ ...membroData, celula_id: celulaId });
...
export async function atualizarMembro(id: string, membroData: Partial<Membro>): Promise<void> {
    ...
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    const { error } = await supabase.from('membros').update(membroData).eq('id', id);
...
export async function getMembro(id: string): Promise<Membro | null> {
    ...
    if (error) { console.error("Erro ao buscar membro:", error); return null; }
    return data;
}
...
export async function listarVisitantes(celulaIdFilter: string | null = null): Promise<Visitante[]> {
    ...
    let query = supabase.from('visitantes').select('id, nome, telefone, endereco, data_primeira_visita, data_ultimo_contato, observacoes, celula_id');
    if (role === 'líder') {
        if (!userCelulaId) return [];
        query = query.eq('celula_id', userCelulaId);
    } else if (role === 'admin' && celulaIdFilter) {
    ...
    const celulasNamesMap = await getCelulasNamesMap(celulaIds, supabase);
    return visitantes.map((v: any) => ({ ...v, celula_nome: celulasNamesMap.get(v.celula_id) || null }));
}
...
export async function getVisitante(id: string): Promise<Visitante | null> {
    ...
    if (error) { console.error("Erro ao buscar visitante:", error); return null; }
    return data;
}
...
export async function atualizarVisitante(id: string, visitanteData: Partial<Visitante>): Promise<void> {
    const { supabase, isAuthorized } = await checkUserAuthorization();
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    const { error } = await supabase.from('visitantes').update(visitanteData).eq('id', id);
    if (error) { throw new Error(`Falha ao atualizar visitante: ${error.message}`); }
...
export async function excluirVisitante(id: string): Promise<void> {
    ...
    if (error) { throw new Error(`Falha ao excluir visitante: ${error.message}`); }
    revalidatePath('/visitantes');
}
...
export async function adicionarReuniao(formData: ReuniaoFormData): Promise<Reuniao> {
    ...
    const { data, error } = await supabase.from('reunioes').insert({ ...formData, celula_id: celulaId }).select().single();
    if (error) {
        throw error;
    }
...
export async function listarReunioes(celulaIdFilter: string | null = null): Promise<ReuniaoComNomes[]> {
    const { supabase, celulaId: userCelulaId, isAuthorized, role } = await checkUserAuthorization();
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    let query = supabase.from('reunioes').select('id, data_reuniao, tema, celula_id, ministrador_principal');
    if (role === 'líder') {
        if (!userCelulaId) return [];
        query = query.eq('celula_id', userCelulaId);
    } else if (role === 'admin' && celulaIdFilter) {
    ...
    const presencasPromises = reunioes.map(async (reuniao) => {
        const [membrosCount, visitantesCount] = await Promise.all([
            supabase.from('presencas_membros').select('id', { count: 'exact', head: true }).eq('reuniao_id', reuniao.id).eq('presente', true),
            supabase.from('presencas_visitantes').select('id', { count: 'exact', head: true }).eq('reuniao_id', reuniao.id).eq('presente', true)
        ]);
        return {
            ...reuniao,
...
export async function duplicarReuniao(reuniaoId: string): Promise<void> {
    const { supabase, isAuthorized } = await checkUserAuthorization();
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    const { data: reuniaoOriginal, error: fetchError } = await supabase.from('reunioes').select('*').eq('id', reuniaoId).single();
    if (fetchError || !reuniaoOriginal) { throw new Error("Reunião original não encontrada."); }
    const { id, created_at, ...newReuniaoData } = reuniaoOriginal;
    newReuniaoData.tema = `${newReuniaoData.tema} (Cópia)`;
    const { error: insertError } = await supabase.from('reunioes').insert(newReuniaoData);
    if (insertError) { throw new Error(`Falha ao duplicar reunião: ${insertError.message}`); }
    revalidatePath('/reunioes');
}
...
export async function listarTodosMembrosComPresenca(reuniaoId: string): Promise<MembroComPresenca[]> {
    const { supabase, isAuthorized } = await checkUserAuthorization();
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    const { data: reuniao, error: reuniaoError } = await supabase.from('reunioes').select('celula_id').eq('id', reuniaoId).single();
    if (reuniaoError || !reuniao) { throw new Error("Reunião não encontrada."); }
    const { data: membros, error: membrosError } = await supabase.from('membros').select('*').eq('celula_id', reuniao.celula_id).order('nome');
    if (membrosError) { throw new Error("Erro ao listar membros."); }
    const { data: presencas, error: presencasError } = await supabase.from('presencas_membros').select('membro_id, presente').eq('reuniao_id', reuniaoId);
    if (presencasError) { throw new Error("Erro ao buscar presenças."); }
    const presencasMap = new Map(presencas.map(p => [p.membro_id, p.presente]));
    return (membros || []).map(membro => ({ ...membro, presente: presencasMap.get(membro.id) || false }));
}
...
export async function listarTodosVisitantesComPresenca(reuniaoId: string): Promise<VisitanteComPresenca[]> {
    ...
    const { data: reuniao, error: reuniaoError } = await supabase.from('reunioes').select('celula_id').eq('id', reuniaoId).single();
    if (reuniaoError || !reuniao) { throw new Error("Reunião não encontrada."); }
    const { data: visitantes, error: visitantesError } = await supabase.from('visitantes').select('*').eq('celula_id', reuniao.celula_id).order('nome');
    if (visitantesError) { throw new Error("Erro ao listar visitantes."); }
    const { data: presencas, error: presencasError } = await supabase.from('presencas_visitantes').select('visitante_id, presente').eq('reuniao_id', reuniaoId);
    if (presencasError) { throw new Error("Erro ao buscar presenças de visitantes."); }
    const presencasMap = new Map(presencas.map(p => [p.visitante_id, p.presente]));
    return (visitantes || []).map(visitante => ({ ...visitante, presente: presencasMap.get(visitante.id) || false }));
}
...
export async function registrarPresencaMembro(reuniaoId: string, membroId: string, presente: boolean): Promise<void> {
    ...
    if (error) { throw new Error(`Erro ao registrar presença de membro: ${error.message}`); }
...
export async function registrarPresencaVisitante(reuniaoId: string, visitanteId: string, presente: boolean): Promise<void> {
    ...
    if (error) { throw new Error(`Erro ao registrar presença de visitante: ${error.message}`); }
    revalidatePath(`/reunioes/presenca/${reuniaoId}`);
}
...
export async function getNumCriancasReuniao(reuniaoId: string): Promise<number> {
    const { supabase, isAuthorized } = await checkUserAuthorization();
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    const { data, error } = await supabase.from('criancas_reuniao').select('numero_criancas').eq('reuniao_id', reuniaoId).single();
    if (error) { return 0; }
...
export async function setNumCriancasReuniao(reuniaoId: string, numCriancas: number): Promise<void> {
    const { supabase, isAuthorized } = await checkUserAuthorization();
    if (!isAuthorized) { throw new Error("Não autorizado."); }
    const { error } = await supabase.from('criancas_reuniao').upsert({ reuniao_id: reuniaoId, numero_criancas: numCriancas }, { onConflict: 'reuniao_id' });
    if (error) { throw new Error(`Erro ao registrar número de crianças: ${error.message}`); }
    revalidatePath(`/reunioes/presenca/${reuniaoId}`);
}
...
export async function getReuniaoDetalhesParaResumo(reuniaoId: string): Promise<ReuniaoDetalhesParaResumo | null> {
    ...
    if (error || !data) { console.error("Erro ao buscar resumo da reunião:", error); return null; }
...
export async function getUserProfile(): Promise<Profile | null> {
    ...
    let celulaNome = null;
    if (celulaId) {
        const { data: celulaData, error: celulaError } = await supabase.from('celulas').select('nome').eq('id', celulaId).single();
        if (!celulaError && celulaData) {
            celulaNome = celulaData.nome;
        }
    }
    return { id: user.id, email: user.email!, role, celula_id: celulaId, celula_nome: celulaNome || undefined };
}
...
export async function uploadPalavraDaSemana(formData: FormData): Promise<void> {
    ...
    if (!file || !titulo || !data_semana) { throw new Error("Campos obrigatórios faltando."); }
    const filePath = `palavra_semana/${file.name}`;
    const { error: uploadError } = await supabase.storage.from('materiais').upload(filePath, file, { upsert: true });
    if (uploadError) { throw new Error(`Erro no upload: ${uploadError.message}`); }
    const { data: { publicUrl } } = supabase.storage.from('materiais').getPublicUrl(filePath);
    const { error: dbError } = await supabase.from('palavra_semana').insert({ titulo, descricao, data_semana, url_arquivo: publicUrl });
...
export async function deletePalavraDaSemana(id: string, url_arquivo: string): Promise<void> {
    ...
    const { error: storageError } = await supabase.storage.from('materiais').remove([filePath]);
    if (storageError) { console.warn(`Aviso: não foi possível deletar o arquivo do storage: ${storageError.message}`); }
    const { error: dbError } = await supabase.from('palavra_semana').delete().eq('id', id);
