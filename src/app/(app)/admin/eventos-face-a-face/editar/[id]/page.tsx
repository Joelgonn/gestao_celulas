// src/app/(app)/admin/eventos-face-a-face/editar/[id]/page.tsx
'use client';
...
import {
    ...
    FaToggleOn,
    FaToggleOff,
    FaChevronDown, // <-- ÍCONE IMPORTADO AQUI!
    FaSearch,      // <-- Necessário para o CustomSelectSheet
    FaCheckCircle, // <-- Necessário para o CustomSelectSheet
    FaTimes        // <-- Necessário para o CustomSelectSheet e InputField
} from 'react-icons/fa';

// --- Reutiliza CustomSelectSheet (copiado da página novo/page.tsx) ---
...
// --- InputField reutilizável (copiado e adaptado da página novo/page.tsx) ---
...
export default function EditarEventoFaceAFacePage() {
        ...
        const fetchEvento = async () => {
            ...
            try {
                ...
                if (!data) {
                    ...
                    setTimeout(() => router.replace('/admin/eventos-face-a-face'), 2000);
                    return;
                }
                setFormData({
                    ...
                    chave_pix_admin: data.chave_pix_admin,
                    ativa_para_inscricao: data.ativa_para_inscricao,
                });
            } catch (e: any) {
                console.error("Erro ao carregar evento:", e);
                addToast(`Erro ao carregar evento: ${e.message}`, 'error');
                setTimeout(() => router.replace('/admin/eventos-face-a-face'), 2000);
            } finally {
                setLoading(false);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast('Evento atualizado com sucesso!', 'success', 3000);
            setTimeout(() => {
                router.push('/admin/eventos-face-a-face');
            }, 1500);
        } catch (e: any) {
            console.error("Erro ao atualizar evento:", e);
            addToast(`Falha ao atualizar evento: ${e.message}`, 'error');
        } finally {
            setSubmitting(false);
        }
    };
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div>
                                <h1 className="text-2xl sm:text-3xl font-bold text-white flex items-center gap-2 sm:gap-3">
                                    ...
                                    Editar Edição Face a Face
                                </h1>
                                <p className="text-purple-100 mt-1 text-sm sm:text-base">
                                    Atualize os detalhes e o status de inscrições do evento
                                </p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                <InputField
                                    ...
                                    required
                                    icon={FaCalendarAlt} // Usando FaCalendarAlt, mas FaCalendarCheck também seria válido aqui.
                            ...
                            {/* Status de Ativação (Toggle Switch) */}
                            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    {formData.ativa_para_inscricao ? <FaToggleOn className="w-5 h-5 text-green-600" /> : <FaToggleOff className="w-5 h-5 text-gray-400" />}
                                    <label htmlFor="ativa_para_inscricao" className="text-sm font-semibold text-gray-700">
                                        Inscrições Ativas
                                    </label>
                                </div>
                                <label htmlFor="ativa_para_inscricao" className="relative inline-flex items-center cursor-pointer">
                                    <input
                                        type="checkbox"
                                        id="ativa_para_inscricao"
                                        name="ativa_para_inscricao"
                                        checked={formData.ativa_para_inscricao}
