                        ...
                        <div className="overflow-y-auto p-4 space-y-2 flex-1 pb-10 sm:pb-4">
                            {allowNone && (
                                <button type="button" onClick={() => { onChange(null); setIsOpen(false); }} className={`w-full text-left px-5 py-4 rounded-2xl flex items-center justify-between transition-all ${!value ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-700 hover:bg-gray-100'}`}>
                                    <span className="text-sm font-bold">Nenhum / Selecione...</span>
...
export default function NovaReuniaoPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isDuplicate) {
                addToast(`Já existe reunião: '${formData.tema}' em ${formatDateForDisplay(formData.data_reuniao)}`, 'warning');
                setSubmitting(false);
                return;
            }
            ...
            addToast('Reunião registrada com sucesso!', 'success');
            setTimeout(() => { router.push('/reunioes'); }, 1500);
            ...
            <div className="bg-gradient-to-br from-emerald-600 to-green-700 pt-8 pb-24 px-4 sm:px-8 shadow-lg">
                <div className="max-w-4xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3"><FaPlus /> Nova Reunião</h1>
                            <p className="text-emerald-100 text-sm font-medium opacity-80 uppercase tracking-widest">Registrar Encontro da Célula</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-10">
                            ...
                            <section className="space-y-6">
                                <h2 className="text-lg font-black text-gray-800 flex items-center gap-2 border-b border-gray-50 pb-4">
                                    <div className="p-2 bg-emerald-100 text-emerald-600 rounded-xl"><FaBookOpen size={16}/></div> Dados do Encontro
                                </h2>
                            ...
                            <section className="space-y-6">
                                <h2 className="text-lg font-black text-gray-800 flex items-center gap-2 border-b border-gray-50 pb-4">
                                    <div className="p-2 bg-purple-100 text-purple-600 rounded-xl"><FaFilePdf size={16}/></div> Material de Apoio (Opcional)
                                </h2>
                                            ...
                                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                                {selectedFile ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <p className="text-[10px] text-gray-400 uppercase tracking-wide">PDF ou PPT (Máx 10MB)</p>
                                                    </>
                                                )}
                                            </div>
                                            <input type="file" className="hidden" accept=".pdf,.ppt,.pptx" onChange={handleFileChange} disabled={uploading || submitting} />
                                        </label>
