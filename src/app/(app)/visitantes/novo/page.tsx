...
const CustomSelectSheet = ({ label, value, onChange, options, icon, placeholder = "Selecione...", searchable = false, required = false, error, disabled = false }: any) => {
    ...
    return (
        <div className={`space-y-1 ${disabled ? 'opacity-50' : ''}`}>
            ...
            <button type="button" onClick={() => !disabled && setIsOpen(true)}
                className={`w-full px-4 py-4 border-2 border-gray-100 rounded-2xl flex items-center justify-between bg-gray-50 transition-all ${disabled ? 'cursor-not-allowed' : 'hover:border-emerald-200'} ${error ? 'border-red-300' : ''}`}>
...
export default function NovoVisitantePage() {
    const [formData, setFormData] = useState<any>({
        nome: '', telefone: '', data_primeira_visita: formatDateForInput(new Date().toISOString()),
        data_nascimento: null, endereco: null, data_ultimo_contato: null, observacoes: null, celula_id: '',
    });
    ...
    useEffect(() => {
        const fetchDependencies = async () => {
            ...
            try {
                ...
                if (cells.length === 1) setFormData((prev: any) => ({ ...prev, celula_id: cells[0].id }));
            } catch (e: any) { addToast('Erro ao carregar células.', 'error'); } finally { setLoading(false); }
        };
        fetchDependencies();
    }, [addToast]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.celula_id) return addToast("Selecione a célula.", "warning");
        setSubmitting(true);
        try {
            await adicionarVisitante({ ...formData, nome: formData.nome.trim(), telefone: normalizePhoneNumber(formData.telefone) || null });
            addToast('Visitante cadastrado com sucesso!', 'success');
            setTimeout(() => router.push('/visitantes'), 1500);
        } catch (e: any) { addToast(`Erro: ${e.message}`, 'error'); setSubmitting(false); }
    };
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                                <FaUserPlus /> Novo Visitante
                            </h1>
                            <p className="text-emerald-100 text-sm font-medium opacity-80 uppercase tracking-widest">Cadastro no sistema</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <InputField label="Nome Completo" name="nome" value={formData.nome} onChange={handleChange} onBlur={handleNameBlur} required icon={FaPen} placeholder="Ex: Maria Silva" />
                                ...
                                {celulasOptions.length > 1 && (
                                    <CustomSelectSheet label="Vincular à Célula" icon={<FaMapMarkerAlt />} value={formData.celula_id} onChange={(v:any) => setFormData((p:any) => ({ ...p, celula_id: v }))} options={celulasOptions} searchable />
                                )}
                            </section>
                            ...
                            <section className="space-y-6">
                                ...
                                <InputField label="Observações e Detalhes" name="observacoes" value={formData.observacoes} onChange={handleChange} type="textarea" icon={FaComments} placeholder="Como o visitante chegou? Tem algum pedido de oração?" />
                            </section>
                            ...
                            <div className="flex flex-col-reverse sm:flex-row justify-end gap-4 pt-8 border-t border-gray-100">
                                ...
                                <button type="submit" disabled={submitting} className="px-10 py-5 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-2xl font-black text-lg shadow-lg shadow-emerald-200 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 flex items-center justify-center gap-3 cursor-pointer uppercase">
                                    {submitting ? <FaSpinner className="animate-spin" /> : <FaSave />} Cadastrar Visitante
                                </button>
                            </div>
                        </form>
                    </div>
