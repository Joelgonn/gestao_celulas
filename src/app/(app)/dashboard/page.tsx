...
export default function DashboardPage() {
    ...
    const chartOptions = { responsive: true, plugins: { legend: { position: 'top' as const, labels: { font: { size: 14, weight: 'bold', }, color: '#333', }, }, title: { display: true, text: 'Média de Presença da Célula (Últimas 8 Semanas)', font: { size: 16, weight: 'bold', }, color: '#333', }, tooltip: { callbacks: { label: function(context: any) { let label = context.dataset.label || ''; if (label) { label += ': '; } if (context.parsed.y !== null) { label += context.parsed.y + '%'; } return label; } } } }, scales: { x: { title: { display: true, text: 'Semana', font: { size: 12, weight: 'bold', }, color: '#555', }, grid: { display: false, }, }, y: { title: { display: true, text: 'Percentual (%)', font: { size: 12, weight: 'bold', }, color: '#555', }, beginAtZero: true, max: 100, ticks: { callback: function(value: any) { return value + '%'; } } }, }, };
    ...
    const newVisitorsTrendChartOptions = { responsive: true, plugins: { legend: { position: 'top' as const, labels: { font: { size: 14, weight: 'bold', }, color: '#333', }, }, title: { display: true, text: 'Tendência de Novos Visitantes (Últimos 6 Meses)', font: { size: 16, weight: 'bold', }, color: '#333', }, }, scales: { x: { title: { display: true, text: 'Mês', font: { size: 12, weight: 'bold', }, color: '#555', }, grid: { display: false, }, }, y: { title: { display: true, text: 'Número de Visitantes', font: { size: 12, weight: 'bold', }, color: '#555', }, beginAtZero: true, ticks: { stepSize: 1, }, }, }, };
