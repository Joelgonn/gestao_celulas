...
export default function EditMembroPage() {
  ...
  useEffect(() => {
    const fetchMembro = async () => {
      setLoading(true);
      try {
        const data = await getMembro(membroId);
        ...
        if (!data) {
          ...
          setLoading(false);
          setTimeout(() => router.replace('/membros'), 2000);
          return;
        }
        ...
        addToast({
          type: 'success',
          title: 'Carregado',
          message: 'Dados do membro carregados com sucesso',
          duration: 3000
        });
        ...
        addToast({
          ...
          message: error?.message || 'Não foi possível carregar os dados do membro'
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      addToast({
        type: 'error',
        title: 'Acesso negado',
        message: 'Você precisa estar logado para editar membros'
      });
      setSubmitting(false);
      return;
    }

    const updatedMembroData = {
      nome: formData.nome,
    ...
    try {
      await atualizarMembro(membroId, updatedMembroData);
      ...
      if (error.code === '23505') {
        addToast({
          type: 'error',
          title: 'Duplicação',
          message: 'Já existe outro membro com este nome na sua célula'
        });
      } else {
        addToast({
          type: 'error',
          title: 'Erro ao atualizar',
          message: error.message || 'Erro desconhecido ao atualizar membro'
        });
      }
    } finally {
      setSubmitting(false);
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-8">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-indigo-100 mt-2">Atualize as informações do membro da célula</p>
              </div>
              <Link 
                href="/membros"
          ...
          <div className="p-6 sm:p-8">
            {loading ? (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-gray-600 font-medium">Carregando dados do membro...</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button 
                  type="submit" 
                  disabled={submitting}
                  className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-4 px-6 rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-[1.02] disabled:scale-100 flex items-center justify-center gap-2"
                >
                  {submitting ? (
                    <>
