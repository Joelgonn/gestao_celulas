...
const CustomSelectSheet = ({ label, value, onChange, options, icon, placeholder = "Selecione...", searchable = false, disabled = false }: any) => {
    ...
    useEffect(() => {
        const fetchDependencies = async () => {
            ...
            try {
                ...
                addToast('Erro ao carregar células.', 'error');
            } finally {
                setLoading(false);
            }
        };
                ...
                <div className="max-w-3xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-emerald-100 text-sm font-medium opacity-80 uppercase tracking-widest">Cadastro de Célula</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <InputField label="Nome Completo" name="nome" value={formData.nome} onChange={handleChange} onBlur={handleNameBlur} required icon={FaPen} placeholder="Ex: João Silva" />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <CustomSelectSheet label="Status Inicial" icon={<FaUserTag />} value={formData.status} onChange={(v:any) => setFormData((p:any) => ({ ...p, status: v }))} options={[{ id: 'Ativo', nome: 'Ativo' }, { id: 'Inativo', nome: 'Inativo' }, { id: 'Em transição', nome: 'Em transição' }]} />
                                    {celulasOptions.length > 1 && (
                                        <CustomSelectSheet label="Vincular à Célula" icon={<FaComments />} value={formData.celula_id} onChange={(v:any) => setFormData((p:any) => ({ ...p, celula_id: v }))} options={celulasOptions} searchable />
                                    )}
                                </div>
                            </section>
                            ...
                            <div className="flex flex-col-reverse sm:flex-row justify-end gap-4 pt-8 border-t border-gray-100">
                                ...
                                <button type="submit" disabled={submitting} className="px-10 py-5 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-2xl font-black text-lg shadow-lg shadow-emerald-200 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 flex items-center justify-center gap-3 cursor-pointer uppercase">
                                    {submitting ? <FaSpinner className="animate-spin" /> : <FaSave />} Adicionar Membro
                                </button>
                            </div>
                        </form>
                    </div>
