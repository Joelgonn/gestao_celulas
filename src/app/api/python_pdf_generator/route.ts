...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // Em desenvolvimento, use a URL local do Next.js para o proxy da função Python:
        // 'http://localhost:3000/api/python_pdf_generator'
        // Em produção no Vercel, o caminho `/api/python_pdf_generator` será resolvido
        // para sua Serverless Function Python.
        // Usamos uma variável de ambiente para flexibilidade. É importante que NÃO seja `NEXT_PUBLIC_`
        // pois esta é uma informação do lado do servidor que não deve ser exposta ao cliente.
        const pythonServiceUrl = process.env.PYTHON_PDF_SERVICE_URL || 'http://localhost:3000/api/python_pdf_generator';
        ...
        const pythonResponse = await fetch(pythonServiceUrl, {
            ...
            headers: {
                ...
                // Se seu serviço Python precisar de autorização (ex: token de API), você adicionaria aqui:
        ...
        // Tenta obter o nome do arquivo do Content-Disposition header, ou usa um nome padrão
        const contentDisposition = headers.get('Content-Disposition');
        const filenameMatch = contentDisposition && contentDisposition.match(/filename="([^"]+)"/);
        const filename = filenameMatch ? filenameMatch[1] : 'report.pdf';

        // Define os headers corretos para download do arquivo PDF
