...
export const ReportAlocacaoLideresDisplay = ({ data }: { data: ReportDataAlocacaoLideres }) => {
    ...
    return (
        <div className="space-y-6">
            {/* Adicionado dark:text-gray-300 para o modo escuro */}
            ...
            {safeLideresAlocados.length > 0 && (
                <div className="space-y-2">
                    {/* Adicionado dark:text-blue-300 para o modo escuro */}
                    ...
                    <table className="min-w-full divide-y divide-gray-200 bg-white shadow-sm rounded-lg dark:bg-gray-800 dark:divide-gray-700"> {/* Classes Dark Mode na tabela */}
                        <thead className="bg-gray-50 dark:bg-gray-700"> {/* Classes Dark Mode no cabeçalho */}
                            <tr>
                                <th className="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-300">Email</th> {/* Classes Dark Mode nos headers */}
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {safeLideresAlocados.map((lider) => (
                                <tr key={lider.id} className="even:bg-gray-50 dark:even:bg-gray-900 dark:odd:bg-gray-800"> {/* Fundo zebrado no dark mode */}
                                    <td className="py-2 px-4 whitespace-nowrap text-gray-800 dark:text-gray-200">{lider.email}</td> {/* Texto legível no dark mode */}
            ...
            {safeLideresNaoAlocados.length > 0 && (
                <div className="space-y-2">
                    {/* Adicionado dark:text-orange-300 para o modo escuro */}
            ...
            {safeCelulasSemLider.length > 0 && (
                <div className="space-y-2">
                    {/* Adicionado dark:text-red-300 para o modo escuro */}
